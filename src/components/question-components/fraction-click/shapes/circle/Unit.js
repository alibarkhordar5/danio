...
const UnitCircle = ({ id, denominator, answerToPut, setUser_answer, colors, selectedColor, max_numerator }) => {
    // ---------------- constants -------------
    const numberOfCircles = Math.ceil(max_numerator / denominator) + 1;
    ...
    svgTags.push(
        <line
            x1={`${2 * CIRCLE_RADIUS + (DIST/2)}`}
            y1={`${-HASHIYE}`}
            x2={`${2 * CIRCLE_RADIUS + (DIST/2)}`}
            y2={`${SVG_HEIGHT}`}
            stroke="red"
            stroke-width="2"
            stroke-dasharray="4"
