...
export default function ProfilePicture({ classInfo, isIndividualUser }) {
    ...
    return (
        <Card sx={{ pt: 8, pb: 4, px: 3 }}>
            ...
            <Stack spacing={1.5} mb={1} direction="row">
                <Iconify color="text.disabled" icon="solar:clipboard-list-bold-duotone" />
                <Typography variant="body2"> پایه : {fGradeToStringFormat(classInfo?.grade) || '-'}</Typography>
            </Stack>

            <Stack spacing={1.5} mb={1} direction="row">
                <Iconify color="text.disabled" icon="solar:calculator-minimalistic-bold-duotone" />
                <Typography variant="body2"> کلاس : {classInfo?.name || '-'}</Typography>
            </Stack>
            {isIndividualUser && (
                <Stack spacing={1.5} mb={1} direction="row">
